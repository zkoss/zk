...
<xsl:param name="TITLE">ZK Unit Test Results.</xsl:param>
    ...
    <html>
       ...
       <script type="text/javascript" language="JavaScript">
        var TestCases = new Array(),
			cur = null,
			path = '<xsl:call-template name="filepath" />';
       ...
       <script type="text/javascript" language="JavaScript"><![CDATA[
		...
		function tracker(name) {
			if (name.match(/^F/i)) {
				...
				var number = name.split(/_|\./);
				var win = window.open("http://sourceforge.net/tracker/index.php?func=detail&aid="+number[1]+"&group_id=152762&atid=785191")
				win.focus();
			} else {
				alert("No Tracker");
			}
        ...
        <body>
            ...
            <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
                <xsl:call-template name="testsuite.test.header2"/>
...
<xsl:template name="filepath">file:///<xsl:param name="base" select="properties/property[@name='basedir']/@value"/>
		<xsl:param name="src" select="properties/property[@name='source.dir']/@value"/>
	<xsl:call-template name="DIR-escape">
		<xsl:with-param name="string1" select="$base"/>
		<xsl:with-param name="string2" select="$src"/>
	</xsl:call-template>
</xsl:template>

<xsl:template name="DIR-escape">
    <xsl:param name="string1"/>
	<xsl:param name="string2"/>
    <xsl:param name="tmp1" select="stringutils:replace(string($string1),'\','/')"/>
    <xsl:param name="tmp2" select="stringutils:replace(string($tmp1),' ','')"/>
    <xsl:value-of select="$tmp2"/>/<xsl:value-of select="$string2"/>
</xsl:template>
        ...
        <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
            ...
            <xsl:for-each select="testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
                ...
                <tr valign="top">
                    ...
                    <td class="host"><xsl:value-of select="$insamepackage/@hostname"/></td>
                </tr>
            </xsl:for-each>
        </table>
        </body>
    ...
    <html>
        ...
        <body>
            ...
            <xsl:if test="count($insamepackage) &gt; 0">
                ...
                <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
                    <xsl:call-template name="testsuite.test.header"/>
                    <xsl:apply-templates select="$insamepackage" mode="print.test2">
                        <xsl:sort select="@name"/>
                    </xsl:apply-templates>
                </table>
                </p>
...
<xsl:template name="testsuite.test.header2">
    <tr valign="top">
        <th width="60%">Name</th>
...
<xsl:template match="testsuite" mode="print.test">
    <tr valign="top">
        ...
        <td class="host" >http://<xsl:apply-templates select="@hostname"/></td>
    </tr>
</xsl:template>

<xsl:template match="testsuite" mode="print.test2">
    <tr valign="top">
        <xsl:attribute name="class">
            <xsl:choose>
                <xsl:when test="@errors[.&gt; 0]">Error</xsl:when>
        ...
        <td><xsl:apply-templates select="@timestamp"/></td>
        <td class="host" ><xsl:apply-templates select="@hostname"/></td>
    </tr>
</xsl:template>
