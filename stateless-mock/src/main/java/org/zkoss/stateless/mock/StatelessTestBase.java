...
/**
 * A base class for Junit to run with Stateless immutable component.
 * @author jumperchen
 */
public class StatelessTestBase {
	...
	public StatelessTestBase() {
	}

	public StatelessTestBase(boolean disableForBenchmarkMode) {
	...
	private static String trimUuidPart(String output, boolean disableForBenchmarkMode) {
		if (disableForBenchmarkMode)
			return output.replaceAll("'z__[^\']*'", "")
				...
				.replaceAll(",\\{cssflex:false}", ",{}")
				.replaceAll("_stateless:true", "") // for IMeshElement
