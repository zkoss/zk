...
public class Carousel extends XulElement {
	 ...
	 * the value is clamped to {@code count - 1} — or 0 when there are no
	 * slides — for both render and getter so callers never observe an index
	 * that doesn't correspond to a real slide.
	 */
	...
	protected void renderProperties(org.zkoss.zk.ui.sys.ContentRenderer renderer) throws java.io.IOException {
		// Ship the same clamped index getActiveIndex() reports, without
		// mutating _activeIndex, so render stays side-effect-free and the
		// stored value can grow back into range when slides attach later. The
		// clamp matters only for ZUL pages where setActiveIndex(N) appears
		// before the N-th carouselitem in the file; the setter and
		// onChildRemoved handle the other paths.
		final int effectiveIdx = getActiveIndex();
