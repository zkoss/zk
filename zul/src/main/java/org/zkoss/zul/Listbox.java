...
public class Listbox extends MeshElement {
	...
	public void addItemToSelection(Listitem item) {
		...
		if (!item.isSelected()) {
			if (!_multiple) {
				...
				if (isItemInCurrentPage(item) && (item.getIndex() < _jsel || _jsel < 0)) {
					_jsel = item.getIndex();
	...
	/**
	 * Checks whether the given item is in the current page.
	 */
	private boolean isItemInCurrentPage(Listitem item) {
		if (!inPagingMold())
			return true;
		boolean isInCurrentPage = false;
		if (item != null && item.getParent() == this) {
			final int page = item.getIndex() / getPageSize();
			isInCurrentPage = page == getActivePage();
		}
