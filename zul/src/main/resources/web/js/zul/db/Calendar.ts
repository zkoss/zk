...
// The five range-highlight CSS classes a caller may have painted on the day
// cells; listed here because only this widget knows when it rebuilt the grid.
	...
	_markCal0(opts?: MarkCalOptions): void {
		...
		if (this._view == 'day') {
			...
			if (!opts || !opts.sameMonth) {
				...
				// A cross-month redraw rebuilds each <td>'s day value and monofs
				// (see below), so a range highlight painted for the old month is
				// now on the wrong cells. Only this widget knows the grid was
				// rebuilt — the caller does not repaint after _markCal — so strip
				// it here. Not on sameMonth updates: those keep the same cells, so
				// a caller's highlight must survive them.
					...
					for (var k = 0; k < 7; ++k, ++cur) {
						...
						if (k == 0 && cur > last)
							...
							if (y >= maxyear && m == 11 && monofs == 1
								...
								// from a PRIOR render. Clear it so nothing keyed off that
								// value — a range highlight, a click — can phantom-tag
								// this stale cell.
