...
function _fixPageSize(wgt: MeshWidget, rows: HTMLCollectionOf<HTMLTableRowElement>): boolean | undefined {
	var ebody = wgt.ebody;
	if (!ebody)
...
export abstract class MeshWidget extends zul.Widget {
	...
	public setPagingPosition(pagingPosition: string, opts?: Record<string, boolean>): this {
		...
		if (o !== pagingPosition || (opts && opts.force)) {
			this.rerender();
		}
	...
	public setAutopaging(autopaging: boolean, opts?: Record<string, boolean>): this {
		...
		if (o !== autopaging || (opts && opts.force)) {
			this.rerender();
		}
	...
	public override setFlexSize_(sz: zk.FlexSize, isFlexMin?: boolean): void {
		...
		if (sz.height !== undefined) {
			if (sz.height == 'auto') {
				...
				return super.setFlexSize_(sz, isFlexMin);
			}
		}
		...
		if (allWidths) {
			...
			if (hdtbl) {
				...
				for (var w = this.ehdfaker!.firstChild; w; w = w.nextSibling) {
					if ((w as HTMLElement).style.display != 'none' && !(w as HTMLElement).id.endsWith('hdfaker-bar')) // B70-ZK-2307 and B70-ZK-2358
						wd += zk.parseInt((w as HTMLElement).style.width);
				}
...
export let Scrollbar = {
	 ...
	 */
	init(wgt: zul.mesh.MeshWidget): zul.Scrollbar {
