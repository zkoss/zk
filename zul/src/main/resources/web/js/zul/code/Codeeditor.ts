 ...
 * `--zk-codeeditor-token-*` custom property (see the theme profiles), with the
...
export class Codeeditor extends zul.Widget {
	...
	override focus_(timeout?: number): boolean {
		// disabled editors are not focusable; CM mounts async so _cm may be absent
		if (this._disabled || !this._cm)
			return false;
		zk(this._cm.contentDOM).focus(timeout);
		return true;
	}
	...
	_baseExtensions(): Extension[] {
		return [
			history(),
			indentUnit.of('\t'), // indent unit = a tab character (used by Ctrl-]/Ctrl-[)
	...
	_mount(): void {
		...
		if (label || labelledby) exts.push(EditorView.contentAttributes.of(aria));
	...
	/** @internal disabled: dim + drop out of the keyboard tab order (Tab skips); readonly stays focusable. */
	_applyDisabledClass(): void {
		this._toggleClass('disabled', this._disabled);
		if (this._cm)
			this._cm.contentDOM.tabIndex = this._disabled ? -1 : 0;
	}
