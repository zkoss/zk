...
/** @internal */
function _disableCarouselDragStart(evt: JQuery.TriggeredEvent): void {
	...
	// preventDefault only — returning false would also stopPropagation, hiding
	// the dragstart from app-level delegated listeners on document.
	if (!jq(evt.target as HTMLElement).closest(_CTRLS).length)
		evt.preventDefault();
}
