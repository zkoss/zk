...
<tr>
	...
	<td colspan="${tab.closable?4:3}" class="${c:cat3(tscls,'tm',suffix)}"></td>
...
<tr height="${empty tab.height ? '22':tab.height}">
	...
	<td align="left" class="${c:cat3(tscls,'mm',suffix)}"><a href="javascript:;" id="${tab.uuid}!a">${tab.imgTag}<c:out value="${tab.label}"/></a></td>
<c:if test="${tab.closable}">
	<td width="11" align="right" class="${c:cat3(tscls,'mm',suffix)}"><img id="${tab.uuid}!close" src="${c:encodeURL('~./zul/img/close-off.gif')}"/></td>
</c:if>
	<td width="3" class="${c:cat3(tscls,'mm',suffix)}"></td>
	<td class="${c:cat3(tscls,'mr',suffix)}"></td>
</tr>
<tr>
	<td colspan="${tab.closable?6:5}" class="${c:cat(tscls,'b')}"></td>
</tr>
</table>
