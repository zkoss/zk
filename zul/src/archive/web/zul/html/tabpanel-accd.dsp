...
<c:if test="${tab.closable}">
	<td width="11" align="right" style="background-image:url(${c:encodeURL(c:cat('~./zul/img/tab/3d-mm',suffix))})"><img id="${tab.uuid}!close" src="${c:encodeURL('~./zul/img/closesm-off.gif')}"/></td>
</c:if>
	<td width="3" style="background-image:url(${c:encodeURL(c:cat('~./zul/img/tab/3d-mm',suffix))})"></td>
