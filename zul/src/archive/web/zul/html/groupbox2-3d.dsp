...
<div class="${zcls}-header">${z:redraw(self.caption, null)}</div>
</div></div></div>
</c:if>
<c:set var="gcExtStyle" value="${c:cat(empty self.caption ? '': 'border-top:0;', self.contentStyle)}"/>
	<div id="${self.uuid}!slide" class="${zcls}-body" ${self.open?'':' style="display:none"'}><div id="${self.uuid}!cave" class="${self.contentSclass} ${zcls}-cnt"${c:attr('style',gcExtStyle)}>
	<c:forEach var="child" items="${self.children}"><c:if test="${self.caption != child}">${z:redraw(child, null)}</c:if></c:forEach>
	</div></div>
<%-- shadow --%>
<div id="${self.uuid}!sdw" class="${zcls}-bl"><div class="${zcls}-br"><div class="${zcls}-bm"></div></div></div>
</div>