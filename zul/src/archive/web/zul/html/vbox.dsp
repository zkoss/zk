...
<table id="${self.uuid}" zk_type="Box"${self.outerAttrs}${self.innerAttrs} cellpadding="0" cellspacing="0">
	...
	<tr id="${child.uuid}!chdextr" valign="top"><td${c:isInstance('com.potix.zul.html.Splitter',child)?self.splitterExteriorAttrs:self.childExteriorAttrs}>${u:redraw(child, null)}</td></tr>
	</c:forEach>
</table>
