...
<table id="${self.uuid}" z.type="zul.box.Box"${self.outerAttrs}${self.innerAttrs} cellpadding="0" cellspacing="0">
	...
	<tr id="${child.uuid}!chdextr"${u:getBoxChildOuterAttrs(child)}><td valign="top"><div id="${child.uuid}!cell" ${u:getBoxChildInnerAttrs(child)}>${z:redraw(child, null)}</div></td></tr>
	</c:forEach>
</table>