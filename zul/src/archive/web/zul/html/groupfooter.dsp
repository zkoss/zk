...
<tr z.type="Grwgpft" id="${self.uuid}"${self.outerAttrs}${self.innerAttrs}>
	...
	<td z.type="Gcl" id="${child.uuid}!chdextr"${u:getColAttrs(self, status.index)}><div id="${child.uuid}!cell" class="gc tail-cell-inner <c:if test="${self.grid.fixedLayout}">overflow-hidden</c:if>">${z:redraw(child, null)}</div></td>
	</c:forEach>
</tr>
