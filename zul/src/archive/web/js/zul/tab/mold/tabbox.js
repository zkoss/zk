...
function (out) {
	...
	if (this.isHorizontalBottom()){
		...
		if (this.isTabscroll()) {
			out.push(
				'<div id="', uuid , '-right" class="', icon, ' ', this.$s('right'), '">' , getIcon('icon-caret-right'),  '</div>',
		...
		if (this.tabs)
			this.tabs.redraw(out);
		if (this.isVertical()) {
			if (this.isTabscroll()) {
				out.push(
					'<div id="', uuid , '-up" class="', icon, ' ', this.$s('up'), '">' , getIcon('icon-caret-up'),  '</div>',
					'<div id="', uuid , '-down" class="', icon, ' ', this.$s('down'), '">' , getIcon('icon-caret-down'),  '</div>');
			}
		}
		if (this.tabpanels)
			this.tabpanels.redraw(out);
	}
