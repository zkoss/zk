...
function (out) {
	...
	if (tabbox.inAccordionMold()) {//Accordion
		if (tabbox.getMold() == 'accordion-lite') {
			out.push('<div class="',zcls,'-outer" id="',uuid,'">',
					'<div id="',tab.uuid,'"',tab.domAttrs_(),'>');

			out.push('<div align="left" class="',tabzcs,'-header">');
			if (tab.isClosable()) {
				out.push('<a id="',tab.uuid,'$close" class="',tabzcs,'-close"></a>');
			}
			out.push('<div href="javascript:;" id="',tab.uuid,'$tl" class="',tabzcs,'-tl">',
					'<div class="',tabzcs,'-tr">',
					'<span class="',tabzcs,'-tm">',
					'<span class="',tabzcs,'-text">',tab.domContent_(),
