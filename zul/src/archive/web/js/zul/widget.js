...
zkPg = {
	...
	init: function (cmp) {
		zkPg.validate = zkInbox.validate;
		zkTxbox.init($real(cmp));
		
		cmp.actpg = parseInt(getZKAttr(cmp, "actpg"));
		cmp.npg = parseInt(getZKAttr(cmp, "numpg"));
	...
	onover: function (evt) {
		...
		if (table.id.indexOf('tb_f') != -1 || table.id.indexOf('tb_p') != -1) { // first, prev
			if (cmp.actpg == 0) return;
		} else if (table.id.indexOf('tb_n') != -1 || table.id.indexOf('tb_l') != -1) { // next, last
			if (cmp.actpg == cmp.npg - 1) return;
		}
