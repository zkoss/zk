...
function (out) {
	...
	out.push('<table id="', this.uuid, '"', this.domAttrs_(),'>',
			'<tr><td align="right" class="', zcls, '-tdl"><div id="', uuid, '-ly" class="', zcls, '-left"/></td>',
				...
				switch(view) {
					case "day" :
						out.push('<span id="', uuid,'-tm">', zk.SMON[m],'</span>,<span id="', uuid,'-ty">', y, '</span>');
						break;
					case "month" :
						out.push('<span id="', uuid,'-tm">', zk.SMON[m],'</span>,<span id="', uuid,'-ty">', y, '</span>');
						break;
					case "year" :
						out.push('<span id="', uuid,'-tyd">', yofs + 1 , '-', yofs + 10, '</span>');
						break;
					case "decade" :						
						out.push('<span id="', uuid,'-tyd">', ydec*1000 , '-', ydec*1000 + 99, '</span>');
						break;
				}
			...
			switch(view) {
				...
				case "decade" :
					out.push('<tr><td colspan="3" ><table id="', uuid, '-mid" class="', zcls, '-calyear" width="100%" border="0" cellspacing="0" cellpadding="0"><tr>');
					var temp = ydec*1000 - 10;
					for (var j = 0 ; j < 12; ++j) {						
						out.push('<td id="', uuid, '-de', j, '" class="', (y > temp && y <= (temp + 9)) ? zcls + '-seld' : '', '"',
								(j == 0 || j == 11) ? 'style="color:gray;"' : '', ' ><a href="javascript:;">', temp, '-<br />', temp + 9, '</a></td>');
						if (j > 0 && (j + 1) % 4 == 0) out.push('</tr><tr>');
						temp += 10;
					}
					out.push('</tr></table></td></tr>');
