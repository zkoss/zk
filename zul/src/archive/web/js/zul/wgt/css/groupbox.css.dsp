...
.z-groupbox .z-groupbox-hl {
	...
	background-image: url(${c:encodeThemeURL('~./${theme}/img/layout/groupbox-y.gif', theme)});
}
...
.z-groupbox-hm {
	background: no-repeat;
	background-position: 0 center;
	background-image: url(${c:encodeThemeURL('~./${theme}/img/layout/groupbox-x.gif', theme)});
}
...
.z-groupbox-3d-hm {
	...
	background-image: url(${c:encodeThemeURL('~./${theme}/img/layout/groupbox-hm.png', theme)});
	overflow: hidden;
	border: 1px solid #C5C5C5;
	zoom: 1;
	cursor: pointer;
...
<c:if test="${zk.ie == 6}">
.z-groupbox-3d-hm {
	background-image: url(${c:encodeThemeURL('~./${theme}/img/layout/groupbox-hm.gif', theme)});
}
</c:if>