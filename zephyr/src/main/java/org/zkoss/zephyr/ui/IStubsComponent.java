...
import org.zkoss.zephyr.ui.util.VolatileComponent;
...
public class IStubsComponent extends StubsComponent {
	...
	public void service(Event event, Scope scope) throws Exception {
		...
		if (stubevt != null && "$rms$".equals(stubevt.getCommand())) {
			...
			// which doesn't contain Zephyr's @Action
	...
	// ZK StubsComponent don't support to find by ID, but we support it in Zephyr
