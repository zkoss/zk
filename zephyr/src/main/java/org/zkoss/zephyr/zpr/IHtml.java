...
public interface IHtml extends IXulElement<IHtml>, IAnyGroup<IHtml> {
	...
	default void renderProperties(ContentRenderer renderer) throws IOException {
		...
		if (cnt.length() > 0) {
			...
			if (rc != null) {
				...
				cwout.write("<div id=\"");
				cwout.write(((ZephyrContentRenderer)renderer).getBinding().getUuid());
			...
			if (cnt == null) {
				renderer.render("content", new JavaScriptValue("zk('" + ((ZephyrContentRenderer) renderer).getBinding()
	...
	/**
	 * Builds an updater of type {@link IHtml} for {@link org.zkoss.zephyr.ui.UiAgent#smartUpdate(Locator, SmartUpdater)}.
