 ...
 * <p>
 * The default parsing engine is to use <tt>zsoup</tt> library.
 * 
 * @author jumperchen
 * @since 8.0.0
 */
public class HtmlTreeBuilder implements TreeBuilder {
	...
	private Namespace getNamespace(Attribute attr) {
		if (attr != null) {
			...
			if (tags.length > 1) {
				Pair<Element, Namespace> pair = _nsMap.get(tags[0]);
				if (pair == null) {
					throw new UiExceptionX("Unknown namespace: [" + attr.getOriginalKey() + "]",
							attr.getOriginalKey());
				}
				return pair.getY();
			}
	...
	private org.zkoss.idom.DocType convert(DocumentType dtype) {
		return new org.zkoss.idom.DocType(dtype.attr("name"), dtype.attr("publicId"),
				dtype.attr("systemId"));
	}
