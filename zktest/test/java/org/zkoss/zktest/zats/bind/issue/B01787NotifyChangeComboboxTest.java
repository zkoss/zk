...
public class B01787NotifyChangeComboboxTest extends ZATSTestCase {
	@Test
	public void test() {
		...
		for (int i = 0; i < answers.size(); i++) {
			...
			comp1s = desktop.queryAll("#combobox comboitem");
			box1s = desktop.queryAll("#box1 label");
			assertEquals(2, comp1s.size());
			assertEquals(answers.get(i)[0], comp1s.get(0).as(Comboitem.class).getLabel());
			assertEquals(answers.get(i)[1], comp1s.get(1).as(Comboitem.class).getLabel());
