...
public class F110_ZK_4305_DaterangeConstraintStringTest extends WebDriverTestCase {
	...
	public void testPlainSimpleConstraintReadsBack() {
		...
		// ZK-4305: the client hint used to be derived from the exact class
		// SimpleDateConstraint only, so this arrived as null.
		assertEquals("no empty",
				getEval("String(zk.Widget.$(jq('$drSimple')[0]).getConstraint())"),
				"the raw value reaches the widget, so the popup can mark bounds");
	}
}
