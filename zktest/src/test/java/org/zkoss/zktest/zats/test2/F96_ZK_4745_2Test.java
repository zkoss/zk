...
public class F96_ZK_4745_2Test  extends WebDriverTestCase {
	...
	public void testJapaneseFormat() {
		...
		waitResponse();
		Assertions.assertEquals("平成31/04/30", db1real.get("value"));
		Assertions.assertEquals("平成 31-04-30", db2real.get("value"));
		Assertions.assertEquals("2019/04/30", db3real.get("value"));

		click(jq("@button:eq(1)"));
		waitResponse();
		Assertions.assertEquals("令和01/05/01", db1real.get("value"));
		Assertions.assertEquals("令和 01-05-01", db2real.get("value"));
		Assertions.assertEquals("2019/05/01", db3real.get("value"));
	}
	...
	public void testISOFormat() {
		...
		waitResponse();
		Assertions.assertEquals("西暦 2019-04-30", db4real.get("value"));

		click(jq("@button:eq(3)"));
		waitResponse();
		Assertions.assertEquals("西暦 2019-05-01", db4real.get("value"));
	}
	...
	public void testENJapaneseFormat() {
		...
		waitResponse();
		Assertions.assertEquals("Heisei 31-04-30", db5real.get("value"));

		click(jq("@button:eq(5)"));
		waitResponse();
		Assertions.assertEquals("Reiwa 01-05-01", db5real.get("value"));
	}
