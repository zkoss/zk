...
public class BookCRUDTest extends WebDriverTestCase {
	@Test
	public void test() {
		connect("/bind/basic/bookCrud.zul");
		...
		Assertions.assertEquals("save book Updated Name 0_2", jq(".msg-label").text());
		Assertions.assertEquals("Updated Name 0_2", jq(".z-listitem-selected>.z-listcell:eq(0)").text().trim());
		...
		click(jq(".z-listitem:eq(3)"));
		waitResponse();
		Assertions.assertNotEquals("the deleted book shall not exist on the same index", deletedName, jq("@textbox:eq(0)").val());
	}
}
