...
public class B80_ZK_1696_tree_2Test extends ZATSTestCase {
	...
	public void noOpenTest(){
		...
		Assertions.assertEquals(1, mytree.getActivePage());
		treeCells = treeAgent.queryAll("treecell");
		Assertions.assertEquals("item c", treeCells.get(0).as(Treecell.class).getLabel());
	...
	public void openBackTest(){
		...
		//check render result after switching back to original model
		treeCells = treeAgent.queryAll("treecell");
		Assertions.assertEquals("item 64", treeCells.get(4).as(Treecell.class).getLabel());
		Assertions.assertEquals("item 65", treeCells.get(5).as(Treecell.class).getLabel());
		Assertions.assertEquals("item 66", treeCells.get(6).as(Treecell.class).getLabel());
	}
