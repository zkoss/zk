...
public class F102_ZK_5600Test extends WebDriverTestCase {
	...
	public void test1() throws Exception {
		...
		click(jq("@button:contains(unexpectable)").eq(0));
		waitResponse();
		verify(logger, times(1)).error(eq("at [{}]"), eq(path), any(Exception.class));
		click(jq(".z-window-close"));
		waitResponse();
		click(jq("@button:contains(unexpectable)").eq(1));
		waitResponse();
		verify(logger, times(2)).error(eq("at [{}]"), eq(path), any(Exception.class));
		click(jq(".z-window-close"));
		waitResponse();

		click(jq("@button:contains(original1)"));
		waitResponse();
		verify(logger, times(3)).error(eq("at [{}]"), eq(path), any(Exception.class));
		click(jq(".z-window-close"));
		waitResponse();
		click(jq("@button:contains(original2)"));
		waitResponse();
		verify(logger, times(4)).error(eq("at [{}]"), eq(path), any(Exception.class));
	}
