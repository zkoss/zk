 ...
 * <li>a request carrying a blank desktop id (empty or whitespace-only) is as
 * incomplete as a missing one, so it must also return 467;</li>
 ...
 * </ul>
 */
public class B110_ZK_6011Test extends WebDriverTestCase {
	...
	public void test() throws Exception {
		...
		// a blank desktop id (dtid present but empty or whitespace-only) is as
		// incomplete as a missing one, so it must also return 467, not a
		// misleading 200.
		assertEquals(SC_INCOMPLETE_REQUEST,
				postStatus(auUri, "application/x-www-form-urlencoded", "dtid=%20"),
				"An AU request with a blank desktop id must return HTTP 467, not 200");
