...
public class B96_ZK_4792Test extends WebDriverTestCase {
	...
	public void test() {
		...
		waitResponse();
		Assert.assertEquals("0", jq("$lb1").text());
		Assert.assertEquals("0", jq("$lb2").text());
		Assert.assertEquals("0", jq("$lb3").text());
		Assert.assertEquals("0", jq("$lb4").text());
		Assert.assertEquals("0", jq("$lb5").text());
	}
}
