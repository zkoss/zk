...
public class F61_ZK_147Test extends WebDriverTestCase {
	...
	public void testNoModel() {
		...
		waitResponse();
		Assertions.assertEquals(2, jq("@listbox:first @listgroup").length());
		...
		waitResponse();
		Assertions.assertEquals(0, jq("@listbox:first @listgroup").length());
	...
	public void testGroupModel() {
		...
		click(jq("@menuitem:contains(Ungroup):visible"));
		waitResponse();
		Assertions.assertEquals(0, jq("@listbox:eq(1) @listgroup").length());
		...
		waitResponse();
		Assertions.assertEquals(2, jq("@listbox:eq(1) @listgroup").length());
	}
