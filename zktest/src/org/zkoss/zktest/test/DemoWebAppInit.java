...
public class DemoWebAppInit implements WebAppInit {
	...
	private void setThemeProperites () {
		String prop = Library.getProperty("org.zkoss.zktest.test.theme.silvergray");
		if (prop == null) {
			...
			try {
				...
				en.hasMoreElements();) {
					...
					if (url.toString().lastIndexOf("silvergray.jar") >= 0) {
						Library.setProperty("org.zkoss.zktest.test.theme.silvergray", "true");
						return;
					}
				}
			} catch (Exception ex) {
			...
			Library.setProperty("org.zkoss.zktest.test.theme.silvergray", "false");
		}
	}
