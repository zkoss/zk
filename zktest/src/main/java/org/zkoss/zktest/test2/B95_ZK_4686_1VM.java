...
public class B95_ZK_4686_1VM {
	...
	public void close(@ContextParam(ContextType.BINDER) BinderCtrl binder,
	                  @BindingParam("myform") Form myform) {
		// workaround
		// Set<String> removedSaveFields = binder.removeSaveFormFieldNames((Form) myform);
		// System.out.println(removedSaveFields);
		Clients.log("Saved Form Field Names in Binder: " + binder.getSaveFormFieldNames(myform));
		resetForm();
	}
