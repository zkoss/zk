...
<tr z.type="Grwgp" id="${self.uuid}"${self.outerAttrs}${self.innerAttrs}>
	...
	<td z.type="Gcl" id="${child.uuid}!chdextr"${u:getColAttrs(self, status.index)}><div id="${child.uuid}!cell" class="${self.zclass}-cnt${self.grid.fixedLayout?' z-overflow-hidden':''}">${status.index == 0?self.imgTag:''}${z:redraw(child, null)}</div></td>
	</c:forEach>
</tr>
