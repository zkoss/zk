	...
	public Object coerceToUi(Object val, Component comp) { //load
		...
		if (val != null) {
			if (xmodel instanceof BindingListModel) {
				...
				if (index >= 0) {
					final Listitem item = lbx.getItemAtIndex(index);
					...
					if (SelectedItemConverter.isOnSelectWhenLoad()) {
						...
						if (item != null && selIndex != index) { // bug 1647817, avoid endless-loop
							...
							items.add(item);
							//bug #2140491
							Executions.getCurrent().setAttribute("zkoss.zkplus.databind.ON_SELECT" + lbx.getUuid(),
									Boolean.TRUE);
				...
				throw new UiException("model of the databind listbox " + lbx
