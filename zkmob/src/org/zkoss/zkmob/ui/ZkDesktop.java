...
public class ZkDesktop implements ZkComponent {
		...
		_itemStateListener = new ItemStateListener() {
			public void itemStateChanged(Item item) {
				...
				if (comp instanceof ChoiceGroup) {
					final ZkChoiceGroup cg = (ZkChoiceGroup) comp;
					final Boolean onSelect = cg.getOnSelect();
					if (onSelect != null) {
						final String[] src = new String[cg.size()];
						int k = 0;
						int j = 0;
						for(Enumeration it = cg.getItems().elements(); j < src.length; ++j) {
							final ZkListItem li = (ZkListItem) it.nextElement();
							if (cg.isSelected(j)) {
								src[k++] = li.getId();
							}
						}
						if (k < src.length) {
							...
							send(new Event(comp.getId(), "onSelect", dst), onSelect.booleanValue());
						} else {
							send(new Event(comp.getId(), "onSelect", src), onSelect.booleanValue());
						}
					}
	...
	public void executeResponse(RTag rtag) {
		...
		if ("setAttr".equals(cmd)) {
			...
			executeAddBfr(data);
		} else if ("addChd".equals(cmd)) {
			executeAddChd(data);
		} else if ("redirect".equals(cmd)) {
			executeRedirect(data);
		} else if ("outer".equals(cmd)) {
			executeOuter(data);
		} else {
...
//			throw new IllegalArgumentException("Unknown response command: "+cmd);
	...
	//addChd command
	private void executeAddChd(String[] data) {
		final String uuid = data[0];
		final ZkComponent parent = lookupUi(uuid);
		createComponents(parent, data[1]);
	}
	...
	private void executeRm(String[] data) {
		...
		if (comp instanceof ZkListItem) {
			...
			if (parent instanceof Item) {
				((Item)parent).removeCommand((Command)comp);
			} else { //Displayable
				((Displayable)parent).removeCommand((Command)comp);
			}
	...
	//after: 0 means add before
	//after: 1 means add after
	private void executeAdd(String[] data, int after) {
		final String uuid = data[0];
		final String rmil = data[1].trim();
		final ZkComponent ref = lookupUi(uuid);
		executeAdd(rmil, ref, after);
	}
	
	private void executeAdd(String rmil, ZkComponent ref, int after) {
		//load the rmil and generate a set of components
		final Vector comps = createComponents(this, rmil);

		if (ref instanceof ZkListItem || ref instanceof ZkCommand) {
			final Listable owner = (Listable) ref.getParent();
	...
	private Vector createComponents(ZkComponent parent, String rmil) {
		InputStream is = null;
		try {
		...
		try {
			...
			throw new IllegalArgumentException("add component failed: " + e.toString());
