...
<xsl:template match="text:span[@text:style-name='Source Text']">
...
<xsl:template match="text:p[@text:style-name='Code Indent 1']">
	<xsl:if test="not(preceding-sibling::*[1][self::text:p[@text:style-name='Code Indent 1']])">
			<xsl:element name="programlisting">
				<xsl:for-each select="child::text:tab-stop" xml:space="preserve">    </xsl:for-each>
				<xsl:value-of select="." />
				<xsl:text disable-output-escaping="no">&#xD;</xsl:text>
				<xsl:apply-templates mode="in-list"
						select="following-sibling::*[1][self::text:p[@text:style-name='Code Indent 1']]" />
			</xsl:element>

	</xsl:if>
</xsl:template>

<xsl:template match="text:p[@text:style-name='Code Indent 1']" mode="in-list">
	<xsl:for-each select="child::text:tab-stop" xml:space="preserve">    </xsl:for-each>
	<xsl:value-of select="." />
	<xsl:text disable-output-escaping="no">&#xD;</xsl:text>
