...
function _zkbPop() {
	...
	if (!_zkbWgts.length) {
		...
		_zkbcrs.stub = zAu.stub;
		zAu.stub = null;
		setTimeout(_zkStart, 0); //timeout to give progressbox a chance to show
	}
}
...
function _zkLdNew0() {
	...
	for (var inf; inf = _zkbcrs.shift();)
		_zkbcr2s.push([inf[0], _zkCreate(null, inf[1])]);

	setTimeout(_zkLdNew1, 0); //timeout to give progressbox a chance to show
}
function _zkLdNew1() {
	var inf = _zkbcr2s.shift();
	if (inf) {
		var wgt = inf[1];
		wgt.replaceHTML(wgt.uuid, inf[0]);

		if (_zkbcr2s.length) {
			setTimeout(_zkLdNew1, 10); //timeout to give progressbox a chance to run
			return;
		}
	}
...
/** Used internally. */
var _zkbWgts = [], zkcurdt, _zkbcrs = [], _zkbcr2s = []; //used to load widget
...
function _zkStart() {
	for (var j = _zkbcrs.length; --j >= 0;) {
		...
		if (!inf.jsLoad) {
			...
			setTimeout(_zkStart, 0); //avoid exec too long
			return;
		}
	}
