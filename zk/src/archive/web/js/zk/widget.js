		...
		if (modal && !wgt) {
			...
			if (!noFocusChange) {
				...
				if (wgt && (wgtVParent = wgt.$n('a')) && jq.nodeName(wgtVParent, 'button', 'input', 'textarea', 'a', 'select', 'iframe')) {
					...
					if (zk.ie) {
						var oldStyle = wgtVParent.style.position;
						wgtVParent.style.position = 'fixed';
					}
					wgt.focus();
					if (oldStyle) wgtVParent.style.position = oldStyle;
				}
				zk._cfByMD = true;
...
zk.NoDom = {
	bind_: function () {
		if (this.getMold() == 'no-dom') {
			var context = this.$getInterceptorContext$();
			this.$supers(this.$class, 'bind_', context.args);
			var node = this.$n('tmp'),
	...
	$n: function (subId) {
		if (!subId && this.getMold() == 'no-dom') {
			var context = this.$getInterceptorContext$();
			var n = this._node;
			if (!n && this.desktop && !this._nodeSolved) {
				this._node = n = this._startNode;
				this._nodeSolved = true;
			}
			context.result = n;
			context.stop = true;
	...
	redraw: function (out) {
		if (this.getMold() == 'no-dom') {
			var context = this.$getInterceptorContext$();
			out.push('<div id="', this.uuid, '-tmp', '" ></div>');
			for (var w = this.firstChild; w; w = w.nextSibling)
				w.redraw(out);
			context.stop = true;
		}
