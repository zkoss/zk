 ...
 *		&lt;listener-class>org.zkoss.zk.ui.util.ThreadLocalListener&lt;/listener-class>
 *	&lt;/listener>
 * </code></pre>
...
public class ThreadLocalListener implements EventThreadInit, EventThreadCleanup, EventThreadResume {
	...
	private static Map _fieldsMap = new HashMap(8); //(class name, String[] of fields)

	private Map _threadLocalsMap = new HashMap(8); //(class name, ThreadLocal_Contents[] for fields)

	public ThreadLocalListener() {
		if (!_init) {
			_init = true;
			//read preference
			final Configuration config = Executions.getCurrent().getDesktop().getWebApp().getConfiguration();
