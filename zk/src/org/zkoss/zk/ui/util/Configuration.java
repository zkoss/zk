...
public class Configuration {
	...
	/** Sets the URI prefix.
	 *
	 * <p>Default: null (no prefix at all)
	 *
	 * <p>The URI prefix is used to prefix all URIs generated by
	 * {@link Execution#encodeURL}.
	 * For example, if the context path is "/zkdemo", the URI "/hello.zul"
	 * and the URI prefix "/proxy", then the generated URL will be
	 * "/proxy/zkdemo/hello.zul".
	 *
	 * <p>The URI prefix is used in a sophisticated environment, such as
	 * <a href="http://en.wikipedia.org/wiki/Reverse_proxy">Reverse Proxy</a>.
	 *
	 * <p>Note: if you want to have more advance control (other than prefix),
	 * you can implement {@link org.zkoss.web.servlet.http.Encodes.URLEncoder}
	 * and then specify it with {@link Encodes#setURLEncoder}.
	 *
	 * @param prefix the URI prefix. If empty, null is assumed (i.e., no prefix
	 * at all). If not empty, 
	 * @since 3.0.1
	 */
	public void setURIPrefix(String prefix) throws UiException {
		if (prefix != null) {
			if (prefix.length() == 0) prefix = null;
			else if (prefix.charAt(0) != '/')
				throw new UiException("The URI prefix must starts with '/', "+prefix);
		}
		_uriPrefix = prefix;
		Encodes.setURLEncoder(_uriPrefix != null ? new URIPrefix(): null);
	}
	/** Returns the URI prefix, or null if no prefix at all.
	 * @since 3.0.1
	 * @see #setURIPrefix
	 */
	...
	private static class URIPrefix implements Encodes.URLEncoder {
		public String encodeURL(ServletContext ctx,
		ServletRequest request, ServletResponse response, String uri) {
			final String prefix = WebManager.getWebManager(ctx)
				.getWebApp().getConfiguration().getURIPrefix();
			return prefix != null && uri != null && uri.startsWith("/") ?
				prefix + uri: uri;
		}
	}
}
