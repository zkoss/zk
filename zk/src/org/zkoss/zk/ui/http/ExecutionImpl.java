...
public class ExecutionImpl extends AbstractExecution {
	 ...
	 */
	public ExecutionImpl(ServletContext ctx, HttpServletRequest request, HttpServletResponse response, Desktop desktop,
			Page creating) {
	...
	public VariableResolver getVariableResolver() {
		if (_resolver == null)
			_resolver = new ExecutionResolver(this, new RequestXelResolver(_ctx, _request, _response) {
	...
	public Evaluator getEvaluator(Page page, Class<? extends ExpressionFactory> expfcls) {
		...
		if (_eval == null || _eval.page != page || _eval.getExpressionFactoryClass() != expfcls)
	...
	public Object evaluate(Component comp, String expr, Class<?> expectedType) {
		return evaluate0(comp, expr, expectedType, comp != null ? comp.getPage() : null);
	}
	...
	private Object evaluate0(Object self, String expr, Class<?> expectedType, Page page) {
		...
		return self instanceof Page ? eval.evaluate((Page) self, expression)
	...
	private boolean dispatch(Writer out, String page, Map params, int mode, boolean include)
		...
		if ((mode != PASS_THRU_ATTR && params != null) || !page.startsWith("~./") || page.indexOf('?') >= 0)
			return false;
		...
		final String attrnm = include ? "org.zkoss.web.servlet.include" : "org.zkoss.web.servlet.forward";
	...
	public String locate(String path) {
		try {
			if (path.startsWith("~./")) {
				path = Servlets.locate(_ctx, _request, ClassWebResource.PATH_PREFIX + path.substring(2),
						Locators.getDefault());
	...
	public PageDefinition getPageDefinition(String uri) {
		...
		final PageDefinition pagedef = ((WebAppCtrl) getDesktop().getWebApp()).getUiFactory()
	...
	public PageDefinition getPageDefinitionDirectly(String content, String ext) {
		//Note: we have to go thru UiFactory (so user can override it)
		return ((WebAppCtrl) getDesktop().getWebApp()).getUiFactory().getPageDefinitionDirectly(newRequestInfo(null),
				content, ext);
	}
	...
	private RequestInfo newRequestInfo(String uri) {
		...
		return new RequestInfoImpl(dt, _request, PageDefinitions.getLocator(getDesktop().getWebApp(), uri));
	}
