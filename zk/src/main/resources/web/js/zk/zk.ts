...
foo.MyClass = zk.$extends(foo.MySuper, {
  multiply: function () {
   return this.$supers(foo.MyClass, 'multiply', arguments);
	...
	$supers(a: typeof ZKObject | string, b: string | unknown[], c?: unknown[]): unknown {
		const [nm, args] = typeof a === 'string' ? [a, b as unknown[]] : [b as string, c!];
		try {
			const sup = Object.getPrototypeOf(
				typeof a === 'string' ? Object.getPrototypeOf(this) : a.prototype
			) as Record<string, (...args: unknown[]) => unknown>;
			return sup[nm].apply(this, args);
		} catch (e) {
			throw nm + ' not in superclass'; //args is the method name
		}
	}
