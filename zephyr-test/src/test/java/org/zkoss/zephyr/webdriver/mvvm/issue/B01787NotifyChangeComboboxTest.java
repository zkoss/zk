...
public class B01787NotifyChangeComboboxTest extends WebDriverTestCase {
	@Test
	public void test() {
		connect();
		...
		for (int i = 0; i < answers.size(); i++) {
			...
			comp1s = jq("$combobox @comboitem");
			box1s = jq("$box1 @label");
			assertEquals(2, comp1s.length());
			assertEquals(answers.get(i)[0], comp1s.eq(0).text().replaceAll(" ", " "));
			assertEquals(answers.get(i)[1], comp1s.eq(1).text().replaceAll(" ", " "));
			assertEquals(2, box1s.length());
			assertEquals(answers.get(i)[2], box1s.eq(0).text().replaceAll(" ", " "));
			assertEquals(answers.get(i)[3], box1s.eq(1).text().replaceAll(" ", " "));
		}
	}
}
