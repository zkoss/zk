...
public class ITreefooterRichlet implements StatelessRichlet {
	...
	public List<IComponent> image() {
		return Arrays.asList(
				...
				ITree.DEFAULT.withTreefoot(ITreefoot.of(ITreefooter.ofImage("/stateless/ZK-Logo.gif").withId("footer")))
		);
	}

	@Action(type = Events.ON_CLICK)
	public void changeImage() {
		UiAgent.getCurrent().smartUpdate(Locator.ofId("footer"), new ITreefooter.Updater().image("/zephyr-test/stateless/ZK-Logo-old.gif"));
	}
}